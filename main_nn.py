...
if __name__ == '__main__':
    ...
    # alpha: 2^(-14) ~ 1
    hls_list = []
    for i in np.power(2, np.arange(1, 6)):
        for j in np.power(2, np.arange(1, 6)):
            for k in np.power(2, np.arange(1, 6)):
                hls_list.append((i, j, k))
    alpha_list = np.power(2, np.arange(-14, 1))
    ...
    for hls in hls_list:
        for alpha in alpha_list:
            for activation in activation_tuple:
                for solver in solver_tuple:
                    ...
                    for i in range(k_fold):
                        ...
                        X_test = testset[i][0]
                        y_test = testset[i][1]
                        pred = classification_algo.neural_network(X_train, y_train, X_test, hls, activation, solver, alpha)
                        accuracy = sklearn.metrics.accuracy_score(y_test, pred)
                        #  print("Run: {}, {}".format(i+1, accuracy))
                        accuracy_list[i] = accuracy
                    print("{0:.3f},hls={1},alpha={2},activation={3},solver={4},NeuralNetwork".format(accuracy_list.mean(),hls,alpha,activation,solver))
